...
//	      Launch the browser
			...
			ct.merge(dc);
			// The above code will dismiss the desired capabilities of unexpected behavior in browser
			...
			WebDriverWait wait = new WebDriverWait(driver, 30); //Adding explicit wait also 
			...
			WebElement charr = driver.findElement(By.xpath("//a[text()='Office Chairs']"));
			ac.moveToElement(charr).click().build().perform();
			Thread.sleep(3000);

//			click Executive Chairs
			WebElement execcharr = driver.findElement(By.xpath("//h5[contains(text(),'Executive Chairs')]"));
			wait.until(ExpectedConditions.elementToBeClickable(execcharr));
			execcharr.click();
